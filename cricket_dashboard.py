...
FC={
    ...
    "BBL":"#ff7675","CPL":"#55efc4","SA20":"#fab1a0",
}
FORMATS=["ODI","Test","T20I","IPL","PSL","WPL","NT20","Ranji","BBL","CPL","SA20"]
...
# ── Load CSVs from GitHub — 1hr cache so live data flows after each push ─
...
<b style='color:#00b894'>⚡ Live updates</b><br>Data auto-refreshes every hour from Cricsheet via GitHub. Hit 🔄 after a match to get fresh records immediately.</div>""",unsafe_allow_html=True)
...
def bar_h(df,x,y,col,scale,title):
    ...
    lm=max(240,int(max_chars*8.5+30)); h=max(420,len(df)*58)
    ...
    fig.update_traces(marker_line_width=0,text=df[x].round(1).astype(str),textposition="outside",
                      textfont=dict(size=10,color=TEXT),cliponaxis=False)
    fig.update_layout(**BASE,height=h,coloraxis_showscale=False,margin=dict(l=lm,r=80,t=48,b=8),bargap=0.32)
    fig.update_yaxes(categoryorder="total ascending",showgrid=False,title="",tickfont=dict(size=12,color=TEXT),automargin=False,tickmode="linear")
...
def bar_v(df,x,y,title,color,h=340):
    ...
    fig.update_xaxes(tickmode="linear",tickangle=-40,showgrid=False,tickfont=dict(size=11),automargin=True)
    fig.update_yaxes(showgrid=True,gridcolor=GRID)
    return fig

def line(df,x,y,title,color,h=260):
    fig=px.line(df,x=x,y=y,markers=True,title=title)
    fig.update_traces(line=dict(color=color,width=2.5),marker=dict(size=7,color=color,line=dict(width=1.5,color=BG)))
...
def page_banner(emoji,title,subtitle,ga,gb,glow):
    st.markdown(f"""<div style="background:linear-gradient(135deg,{ga} 0%,{gb} 100%);border-radius:16px;padding:22px 24px;margin:0 0 20px 0;border:1px solid {glow}44;overflow:hidden">
  <div style="display:flex;align-items:center;gap:14px">
    <div style="font-size:40px">{emoji}</div>
    <div><div style="color:#fff;font-size:20px;font-weight:800">{title}</div>
    <div style="color:#aabbcc;font-size:13px;margin-top:3px">{subtitle}</div></div>
  </div></div>""",unsafe_allow_html=True)

# ── Extended aliases — men + women + domestic ─────────────────────────────
NAME_ALIASES={
...
def get_wiki(cricsheet_name, search_name):
    try:
        ...
        return {"title":data.get("title",page_title),"bio":bio,"img":img,
                ...
                "ipl_debut":"","psl_debut":"","wpl_debut":"","nt20_debut":"","ranji_debut":"",
...
def show_player_card(cricsheet_name, search_name, fmt="ODI", compact=False):
    """Show Wikipedia-powered player card. compact=True for side-by-side layout."""
  ...
  <div style="color:#aaa;font-size:13px">📖 Profile unavailable</div></div>""",unsafe_allow_html=True)
        return
    img_w = 88 if compact else 110
    img_h = 108 if compact else 135
    img_html=""
    if card["img"]:
    ...
    fmt_key={"ODI":"odi_debut","Test":"test_debut","T20I":"t20_debut","IPL":"ipl_debut",
             "PSL":"psl_debut","WPL":"wpl_debut","NT20":"nt20_debut","Ranji":"ranji_debut"}.get(fmt,"odi_debut")
...
FORMAT_META={
    "ODI":("🌐","#00b894","#00cec9"),"Test":("🏛️","#0984e3","#74b9ff"),
    "T20I":("⚡","#d63031","#ff7675"),"IPL":("🏏","#e17055","#fdcb6e"),
    "PSL":("🟣","#6c5ce7","#a29bfe"),"WPL":("💜","#fd79a8","#fdcb6e"),
    "NT20":("🇵🇰","#00cec9","#81ecec"),"Ranji":("🇮🇳","#fdcb6e","#ffeaa7"),
    "BBL":("🔥","#ff7675","#fab1a0"),"CPL":("🌴","#55efc4","#00b894"),
    "SA20":("🦁","#fab1a0","#fd79a8"),
}
...
if section=="🔍 Player Search":
  ...
  <p style="color:#8899bb;font-size:14px;margin:0 0 20px">Ball-by-ball records · All major formats</p>
  <div style="display:flex;justify-content:center;gap:8px;flex-wrap:wrap;margin-bottom:20px">{badges}</div>
  <p style="color:#556;font-size:13px;margin:0">Try: <span style="color:#00b894;font-weight:600">Babar</span> · <span style="color:#0984e3;font-weight:600">Kohli</span> · <span style="color:#fd79a8;font-weight:600">Smriti</span> · <span style="color:#00cec9;font-weight:600">Shaheen (NT20)</span> · <span style="color:#fdcb6e;font-weight:600">Rohit (Ranji)</span></p>
</div>""",unsafe_allow_html=True)

    name=st.text_input("🔍  Search player","",placeholder="e.g. Babar, Kohli, Smriti, Harmanpreet, Shaheen...")
    if name:
        sname=resolve(name)
        ...
        if not avl: st.error(f"No data for '{name}'. Check spelling or ensure this format is in your GitHub data."); st.stop()
        ...
        if len(bat)==0 and len(bowl)==0:
            st.warning(f"No {fmt} data for '{name}'. Ensure WPL/NT20/Ranji formats are included in your notebook run.")
...
# ══ 2. HEAD TO HEAD ════════════════════════════════════════════════════════
    ...
    if n1 and n2:
        ...
        if len(b1)==0 or len(b2)==0:
            ...
            # ── Side-by-side player cards with compact=True to prevent overflow ──
            ...
            for title,ml in [("🏏 Volume",["runs","fours","sixes"]),
                ...
                fig.update_layout(**BASE,barmode="group",title=title,
                                  height=max(200,len(ml)*110),margin=dict(l=120,r=90,t=48,b=8))
            ...
            if len(by1)>0 and len(by2y)>0:
                ...
                fy=px.line(combined,x="year",y="runs",color="player",markers=True,title=f"Runs per Year — {fmt}",
...
# ══ 3. PLAYER VS VENUE ═════════════════════════════════════════════════════
    ...
    if name:
        ...
        if len(src)==0: st.error("Player not found!")
            ...
            fmt=st.radio("Format",avail(src,"format"),horizontal=True); df_v=src[src["format"]==fmt]
            if st_=="Batting":
...
# ══ 4. PLAYER VS OPPONENT ══════════════════════════════════════════════════
    ...
    if name:
        ...
        if len(src)==0: st.error("Player not found!")
            ...
            fmt=st.radio("Format",avail(src,"format"),horizontal=True); df_o=src[src["format"]==fmt]
            if st_=="Batting":
                m=st.selectbox("Metric",["runs","average","strike_rate","fours","sixes"])
    ...
    if mt=="Batter":
        ...
        if name:
            ...
            if len(src)==0: st.error("Not found!")
                ...
                fmt=st.radio("Format",avail(src,"format"),horizontal=True); df_m=src[src["format"]==fmt]
