...
def load():
    return (pd.read_csv("cricket_batting_stats.csv"),
            ...
            pd.read_csv("cricket_form_ratings.csv"),
            pd.read_csv("cricket_bat_similarity.csv"),
            pd.read_csv("cricket_bowl_similarity.csv"),
            pd.read_csv("cricket_bat_innings.csv"),
            pd.read_csv("cricket_bowl_innings.csv"))

(batting,bowling,bat_fmt,bowl_fmt,bat_yr,bowl_yr,
 bat_ven,bat_opp,bowl_ven,bowl_opp,bvb,wvb,
 form_df,bat_sim,bowl_sim,bat_inn,bowl_inn) = load()
...
# ══ 9. FORM & RATINGS ══════════════════════════════════
    ...
    page_banner("🔥","Form & Ratings","Who is in form right now? Last 2 seasons vs career average","#1a0800","#2e1500","#e17055")
    fmt = st.radio("Format",FORMATS,horizontal=True)
    src = form_df[form_df["format"]==fmt].copy()

    tab1,tab2 = st.tabs(["🔥 On Fire","📉 Out of Form"])
    with tab1:
    ...
    ch(fig2)
    st.caption("Player Score = weighted formula: Average 30% · Strike Rate 25% · Boundary% 20% · Runs 15% · Non-dot% 10%")
