...
def bar_h(df,x,y,col,scale,title):
    ...
    lm=max(160,int(max_chars*7.5)+20)
    h=max(420,len(df)*52)
    ...
    fig.update_layout(**BASE,height=h,coloraxis_showscale=False,
                      margin=dict(l=lm,r=50,t=48,b=8),bargap=0.35)
...
def get_wiki(cricsheet_name, search_name):
    ...
    try:
        ...
        debut  = extract_field(wikitext, ["international debut","debut","testdebutdate","odidebutdate"])
        role   = extract_field(wikitext, ["role","batting style","batting_style","bowling style","bowling_style"])
        nation = extract_field(wikitext, ["country","nationality","national side"])

        return {"title": data.get("title", page_title),
                "bio": bio, "img": img,
                "born": born[:60] if born else "",
                "debut": debut[:60] if debut else "",
                "role": role[:60] if role else "",
...
<div style="background:linear-gradient(135deg,#1a1f3a,#0f1117);border-radius:16px;
  ...
  <div style="color:#aaa;font-size:13px">📖 Player profile unavailable — Wikipedia not reachable</div>
    ...
    pills = ""
    if card["born"]:
        pills += f'<span style="background:#1e2a3a;color:#8899bb;padding:3px 10px;border-radius:12px;font-size:12px;margin-right:6px">🎂 {card["born"]}</span>'
    if card["nation"]:
        pills += f'<span style="background:#1e2a3a;color:#8899bb;padding:3px 10px;border-radius:12px;font-size:12px;margin-right:6px">🌍 {card["nation"]}</span>'
    if card["role"]:
        pills += f'<span style="background:#1e2a3a;color:#8899bb;padding:3px 10px;border-radius:12px;font-size:12px;margin-right:6px">🏏 {card["role"]}</span>'
    if card["debut"]:
        pills += f'<span style="background:#1e2a3a;color:#8899bb;padding:3px 10px;border-radius:12px;font-size:12px">🎯 Debut: {card["debut"]}</span>'
    ...
    if n1 and n2:
        ...
        if len(b1)==0 or len(b2)==0:
            ...
            if len(by1)>0 and len(by2y)>0:
                ...
                fy.update_layout(**BASE,height=300,margin=M_DEFAULT); st.plotly_chart(fy,**CFG)
