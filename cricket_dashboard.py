...
def get_wiki(cricsheet_name,search_name):
    try:
        q=WIKI_NAMES.get(cricsheet_name,search_name+" cricketer")
        r=requests.get("https://en.wikipedia.org/w/api.php",params={
            "action":"query","list":"search","srsearch":q,"format":"json","utf8":1},timeout=5).json()
        ...
        r2=requests.get("https://en.wikipedia.org/w/api.php",params={
            ...
            "pithumbsize":300,"format":"json","utf8":1},timeout=5).json()
        page=next(iter(r2.get("query",{}).get("pages",{}).values()))
        extract=page.get("extract","")
        sents=[s.strip() for s in extract.replace("\n"," ").split(".") if len(s.strip())>20]
        bio=". ".join(sents[:3])+"." if sents else ""
...
def show_player_card(cricsheet_name,search_name):
    ...
    img_html=""
    if card["img"]:
        img_html=f'<img src="{card["img"]}" style="width:110px;height:130px;object-fit:cover;border-radius:12px;border:2px solid #2d3561;flex-shrink:0;box-shadow:0 4px 16px #0008">'
    st.markdown(f"""
<div style="background:linear-gradient(135deg,#1a1f3a,#0f1117);border-radius:16px;
  ...
  <div style="flex:1;min-width:0">
    <div style="color:#fff;font-size:20px;font-weight:800;margin-bottom:6px">{card["title"]}</div>
