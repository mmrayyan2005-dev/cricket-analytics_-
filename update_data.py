"""
Cricket Analytics — Auto Data Updater
======================================
Runs the full data pipeline (download → clean → compute stats → ML → push to GitHub).
Designed to run in GitHub Actions on a schedule, keeping your dashboard data fresh.

Usage:
  python update_cricket_data.py

Required environment variables (set as GitHub Secrets):
  DATA_PUSH_TOKEN  — GitHub Personal Access Token with repo scope
  DATA_GITHUB_USER — Your GitHub username (e.g. mmrayyan2005-dev)
  DATA_GITHUB_REPO — The repo containing the CSVs (e.g. cricket-analytics_-)
"""

import os, sys, urllib.request, zipfile, base64, requests
...
# ── Config ────────────────────────────────────────────────────────────────────
GITHUB_TOKEN = os.environ.get("DATA_PUSH_TOKEN", "")
GITHUB_USER  = os.environ.get("DATA_GITHUB_USER", "")
GITHUB_REPO  = os.environ.get("DATA_GITHUB_REPO", "")
BRANCH       = "main"

if not all([GITHUB_TOKEN, GITHUB_USER, GITHUB_REPO]):
    print("❌ Missing env vars: DATA_PUSH_TOKEN, DATA_GITHUB_USER, DATA_GITHUB_REPO")
    sys.exit(1)
...
downloads = [
    ...
    ("nt20_data", ["nat20_male_csv2.zip", "smat_male_csv2.zip",
                   "pknt20_male_csv2.zip", "national_t20_male_csv2.zip"]),
]

def try_download(folder, filenames):
    for fname in filenames:
        try:
            urllib.request.urlretrieve(CRICSHEET_BASE + fname, fname)
...
def load_format(folder, label):
    ...
    files = [f for f in os.listdir(folder)
             if not f.endswith("_info.csv") and f.endswith(".csv")]
    dfs = []
...
bowl_milestones = bowl_milestones.merge(
    best_fig[["bowler", "format", "best_bowling"]], on=["bowler", "format"], how="left"
)
...
print("\n📈 Step 6 — Split tables (yearly, venue, opponent, matchup)...")
...
bowling_yearly = df.groupby(["bowler", "year", "format"]).agg(
    ...
    wickets=("is_wicket", "sum"), matches=("match_id", "nunique"),
    dot_balls=("is_dot", "sum"),
).reset_index()
...
# ── Step 7: ML — similarity, form ratings, player score ──────────────────────
print("\n🤖 Step 7 — ML: clustering, form ratings, player scores...")
...
# ── Step 8: Push all CSVs to GitHub ──────────────────────────────────────────
print(f"\n🚀 Step 8 — Pushing CSVs to github.com/{GITHUB_USER}/{GITHUB_REPO}...\n")

def push_csv(df_out, filename):
    ...
    payload = {"message": f"Auto-update {filename} [{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}]",
...
files_to_save = {
    ...
    "cricket_bat_similarity.csv":    bat_ml[["striker", "format", "cluster", "average",
                                              "strike_rate", "boundary_pct", "dot_pct",
                                              "runs", "player_score"]],
    "cricket_bowl_similarity.csv":   bowl_ml[["bowler", "format", "cluster", "wickets",
                                               "economy", "average", "dot_pct"]],
}

# Also push a last_updated timestamp file
ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
ts_url     = f"https://api.github.com/repos/{GITHUB_USER}/{GITHUB_REPO}/contents/last_updated.txt"
ts_headers = {"Authorization": f"token {GITHUB_TOKEN}", "Accept": "application/vnd.github.v3+json"}
ts_r       = requests.get(ts_url, headers=ts_headers)
ts_sha     = ts_r.json().get("sha") if ts_r.status_code == 200 else None
ts_payload = {"message": f"Timestamp update {ts}",
               "content": base64.b64encode(ts.encode()).decode(),
               "branch": BRANCH}
if ts_sha:
    ts_payload["sha"] = ts_sha
requests.put(ts_url, headers=ts_headers, json=ts_payload)
print(f"  ✓ last_updated.txt — {ts}")

for fn, df_out in files_to_save.items():
    push_csv(df_out, fn)

print(f"\n✅ All done! Data updated at {ts}")
