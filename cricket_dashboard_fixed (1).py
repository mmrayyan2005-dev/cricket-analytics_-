...
def bar_h(df,x,y,col,scale,title):
    # measure longest label precisely — 7.5px per char + 24px padding, min 220px
    max_chars = int(df[y].astype(str).str.len().max()) if len(df)>0 else 20
    lm = max(220, int(max_chars * 7.5 + 24))
    h  = max(420, len(df) * 58)
    fig=px.bar(df,x=x,y=y,orientation="h",color=col,color_continuous_scale=scale,title=title)
    fig.update_traces(marker_line_width=0, texttemplate="%{x:.1f}", textposition="outside",
                      textfont=dict(size=10, color=TEXT))
    fig.update_layout(**BASE,height=h,coloraxis_showscale=False,
                      margin=dict(l=lm,r=80,t=48,b=8),bargap=0.3)
    fig.update_yaxes(categoryorder="total ascending",showgrid=False,title="",
                     tickfont=dict(size=12,color=TEXT),automargin=True,
                     tickmode="linear", ticklabeloverflow="allow")
...
def get_wiki(cricsheet_name, search_name):
    ...
    try:
        ...
        # parse a date template {{dts|yyyy|mm|dd}} or {{date|...}} or {{birth date|...}}
        def parse_date_template(val):
            ...
            # Strip any remaining templates like {{dts|...}} and grab numbers
            val_clean = _r2.sub(r"\{\{[^}]*\}\}", lambda mo: mo.group(0).replace("|"," "), val)
            val_clean = _r2.sub(r"[{}\[\]]", " ", val_clean).strip()
            # yyyy|mm|dd or yyyy mm dd pattern
            m = _r2.search(r"(\d{4})\D+(\d{1,2})\D+(\d{1,2})", val_clean)
            if m:
                try: return f"{int(m.group(3))} {months[int(m.group(2))]} {m.group(1)}"
                except: pass
            # dd month_name yyyy
            m2 = _r2.search(r"(\d{1,2})\s+([A-Za-z]+)\s+(\d{4})", val_clean)
            if m2:
                mon = m2.group(2).lower()[:3].capitalize()
                return f"{int(m2.group(1))} {mon} {m2.group(3)}"
            # month_name dd yyyy
            m3 = _r2.search(r"([A-Za-z]+)\s+(\d{1,2}),?\s+(\d{4})", val_clean)
            if m3:
                mon = m3.group(1).lower()[:3].capitalize()
                return f"{int(m3.group(2))} {mon} {m3.group(3)}"
            # just a year
            m4 = _r2.search(r"\d{4}", val_clean)
            return val_clean.strip() if m4 else ""

        odi_debut  = parse_date_template(extract_field(wikitext, [
            "odidebutdate","ODIdebutdate","odi_debut_date","odi debut date",
            "ODI debut","odidebut","debut_odi"]))
        test_debut = parse_date_template(extract_field(wikitext, [
            "testdebutdate","Testdebutdate","test_debut_date","test debut date",
            "Testdebut","testdebut","debut_test"]))
        t20_debut  = parse_date_template(extract_field(wikitext, [
            "t20idebutdate","T20Idebutdate","T20debutdate","t20_debut_date",
            "t20debut","T20Idebut","debut_t20i","debut_t20"]))
        any_debut  = parse_date_template(extract_field(wikitext, [
            "debutdate","debut_date","internationaldebutdate","international_debut_date",
            "intdebutdate","debut"]))
        ...
        if len(b1)==0 or len(b2)==0:
            ...
            for title,ml in [("🏏 Volume",["runs","fours","sixes"]),
                ...
                fig.add_trace(go.Bar(name=p2n,y=pretty,x=v2,orientation="h",
                    ...
                    textfont=dict(size=11,color=TEXT),cliponaxis=False))
                all_vals = v1 + v2
                xmax = max(all_vals) * 1.22 if max(all_vals) > 0 else 10
                fig.update_layout(**BASE,barmode="group",title=title,
                                  height=max(220,len(ml)*110),
                                  margin=dict(l=120,r=80,t=48,b=8))
